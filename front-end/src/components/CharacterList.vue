...
export default {
  ...
  methods: {
    ...
    removeFromTeam(character) {
        ...
        this.$root.$data.fightTeam = this.$root.$data.fightTeam.filter(person => person !== character)
        character.inParty = 0;
    },
    inParty(character) {
      for(var j=0; j < this.$root.$data.team.length; j++) {
          if(character.name === this.$root.$data.team[j].name) {
            return true;
          }
        }
      return false;
